...
- Route ordinary page creation, ordinary iteration, repair, redo, and selected-block follow-up to the workspace-local `page-builder-guided-generation` skill.
...
- Ordinary page generation and ordinary iteration may refine an existing CMS region only while its current binding identity stays unchanged.
