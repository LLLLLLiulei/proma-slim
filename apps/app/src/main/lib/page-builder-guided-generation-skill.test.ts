...
describe('page-builder-guided-generation skill docs', () => {
  ...
  test('documents downstream design skill orchestration', () => {
    ...
    expect(tasteSkill).toContain('the following rules are mandatory for PageBuilder work')
    expect(tasteSkill).toContain('workspace-files/index.html')
    expect(tasteSkill).toContain('plain HTML/CSS/JS authoring')
    expect(tasteSkill).toContain('workspace-files/assets/styles.css')
    expect(tasteSkill).toContain('workspace-files/assets/script.js')
    expect(tasteSkill).toContain('Default to zero external dependencies')
    expect(tasteSkill).toContain('https://unpkg.com/')
    expect(tasteSkill).toContain('Do not use `@latest`')
    ...
    expect(tasteSkill).not.toContain('React or Next.js')
    expect(tasteSkill).not.toContain('Tailwind CSS for 90%')
    expect(tasteSkill).not.toContain('npm install')
    expect(tasteSkill).not.toContain('Framer Motion')
  ...
  test('documents taste-skill as static HTML-first worker with unpkg-only optional dependencies', () => {
    const tasteSkill = readRelativeText('../../../default-skills/taste-skill/SKILL.md')

    expect(tasteSkill).toContain('HTML-first static visual worker')
    expect(tasteSkill).toContain('write the page entry to `workspace-files/index.html`')
    expect(tasteSkill).toContain('write first-party CSS to `workspace-files/assets/styles.css`')
    expect(tasteSkill).toContain('write first-party JavaScript to `workspace-files/assets/script.js`')
    expect(tasteSkill).toContain('Use relative paths')
    expect(tasteSkill).toContain('Do not create a package-based frontend project')
    expect(tasteSkill).toContain('Default to zero external dependencies')
    expect(tasteSkill).toContain('load it only from `https://unpkg.com/`')
    expect(tasteSkill).toContain('Pin a concrete version')
    expect(tasteSkill).toContain('Do not output package-manager install commands')
    expect(tasteSkill).toContain('If an optional CDN dependency fails')
    expect(tasteSkill).not.toContain('Use Tailwind CSS (v3/v4) for 90% of styling')
    expect(tasteSkill).not.toContain('Dependency Verification')
    expect(tasteSkill).not.toContain('Default to Server Components')
  })

  test('documents taste-skill design quality and CMS boundaries for static authoring', () => {
    const tasteSkill = readRelativeText('../../../default-skills/taste-skill/SKILL.md')

    expect(tasteSkill).toContain('Silently infer the page kind')
    expect(tasteSkill).toContain('Do not print a design read')
    expect(tasteSkill).toContain('DESIGN_VARIANCE')
    expect(tasteSkill).toContain('MOTION_INTENSITY')
    expect(tasteSkill).toContain('VISUAL_DENSITY')
    expect(tasteSkill).toContain('avoid common AI-default patterns')
    expect(tasteSkill).toContain('Run this lightweight preflight')
    expect(tasteSkill).toContain('Copy self-audit')
    expect(tasteSkill).toContain('CMS source tags')
    expect(tasteSkill).toContain('Vue template syntax only inside')
    expect(tasteSkill).toContain('Do not add page-wide Vue runtime')
    expect(tasteSkill).toContain('ordinary page regions in plain HTML/CSS/JS')
  })

  test('documents taste-skill v2 anti-template discipline (layout hard rules, production tells, em-dash ban)', () => {
    const tasteSkill = readRelativeText('../../../default-skills/taste-skill/SKILL.md')

    // Layout hard rules (mechanically checkable, zero-dependency)
    expect(tasteSkill).toContain('Eyebrow restraint')
    expect(tasteSkill).toContain('at most 1 eyebrow per 3 sections')
    expect(tasteSkill).toContain('ceil(sectionCount / 3)')
    expect(tasteSkill).toContain('Hero stack discipline')
    expect(tasteSkill).toContain('Zigzag alternation cap')
    expect(tasteSkill).toContain('Bento cell count rule')
    expect(tasteSkill).toContain('No duplicate CTA intent')
    expect(tasteSkill).toContain('Split-header ban')

    // Production-test AI tells (from real LLM-page tests)
    expect(tasteSkill).toContain('Version labels in the hero')
    expect(tasteSkill).toContain('Section-number eyebrows')
    expect(tasteSkill).toContain('middle-dot')
    expect(tasteSkill).toContain('Div-based fake product UI')
    expect(tasteSkill).toContain('Quietly in use at')
    expect(tasteSkill).toContain('Weather or locale strips')
    expect(tasteSkill).toContain('Scroll cues')
    expect(tasteSkill).toContain('credit captions used as decoration')
    expect(tasteSkill).toContain('Version footers on marketing pages')

    // Em-dash: do not generate, but preserve user-provided copy
    expect(tasteSkill).toContain('em-dash')
    expect(tasteSkill).toContain('Do not introduce em-dashes')
    expect(tasteSkill).toContain('User-provided brand copy that already contains an em-dash may be kept')

    // Expanded preflight
    expect(tasteSkill).toContain('Eyebrow count is at most')
    expect(tasteSkill).toContain('No freshly generated em-dash')
  })
