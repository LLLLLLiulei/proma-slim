...
import type { AgentEvent, AgentProviderAdapter, AgentQueryInput, TypedError } from '@ai-page-builder/shared'
...
describe('AgentOrchestrator friendly error handling', () => {
  ...
  test('auto-compacts and retries when a catch-path context-window-limit error is raised', async () => {
    ...
    expect(emittedEventTypes.filter((type) => type === 'compacting')).toHaveLength(1)
    expect(emittedEventTypes.filter((type) => type === 'compact_complete')).toHaveLength(1)
    expect(emittedEventTypes.indexOf('compacting')).toBeLessThan(emittedEventTypes.indexOf('compact_complete'))
    expect(onErrorMessages).toEqual([])
