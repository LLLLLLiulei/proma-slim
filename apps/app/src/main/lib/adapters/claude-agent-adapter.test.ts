...
describe('ClaudeAgentAdapter SDK option pass-through', () => {
  ...
  test('treats SDK errors after adapter abort as a normal stream end', async () => {
    mock.module('@anthropic-ai/claude-agent-sdk', () => ({
      query: async function* (input: { options: { abortController: AbortController } }) {
        ...
        throw new Error('arbitrary sdk failure after local abort')
      },
    }))
  ...
  test('still surfaces SDK errors when the stream was not locally aborted', async () => {
    mock.module('@anthropic-ai/claude-agent-sdk', () => ({
      query: async function* () {
        yield {
          type: 'auth_status',
          isAuthenticating: true,
          output: ['Streaming before failure'],
        }

        throw new Error('upstream failed while still active')
      },
    }))
    ...
    const iterator = adapter.query({
      ...
      prompt: 'Fail this stream',
      cwd: '/tmp/workspace/session-active-error',
      sdkCliPath: '/tmp/claude.js',
      executable: { type: 'node', path: '/usr/bin/node' },
      executableArgs: [],
      env: {},
    ...
    await expect(iterator.next()).resolves.toEqual({
      ...
      value: {
        type: 'status_notice',
        level: 'info',
        message: 'Streaming before failure',
      },
    })

    await expect(iterator.next()).rejects.toThrow('upstream failed while still active')
  })
