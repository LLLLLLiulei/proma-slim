...
describe('page-builder inline text service', () => {
  ...
  test('savePageBuilderInlineText returns preview metadata recomputed from the latest html', () => {
    ...
    writeFileSync(
      ...
      '<!doctype html><html><body><section id="hero"><h1>旧标题</h1></section><section data-proma-block-id="pb_blk_news"><cms-content site-id="14" catalog-id="16"><template v-slot:default="{ items }"><article v-for="item in items" :key="item.id">{{ item.title }}</article></template></cms-content></section></body></html>',
      'utf-8',
    )
