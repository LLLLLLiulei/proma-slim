...
describe('page-builder cms rendering apply tool', () => {
  test('applies catalog-nav bindings through the html mutation pipeline and preserves block ids', () => {
    ...
    expect(result.generatedHtml).toContain('<!-- PROMA CMS REGION: 该 cms-catalog 是宿主管理的 CMS binding source tag；不要为这个区域再次引入整页 Vue runtime；不要添加 Vue CDN、importmap、createApp、Vue.createApp 或 app.mount(...)；Vue 模板语法只允许出现在这个 CMS 标签内部的 slot templates 中；如需更换 CMS 数据来源或 binding props，请重新走已确认的 CMS apply 流程，不要直接手写改绑。 -->')
