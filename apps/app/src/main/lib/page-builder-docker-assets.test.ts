...
describe('page-builder docker assets', () => {
  test('compose defines an optional internal playwright sidecar and docker runtime toggles', () => {
    ...
    expect(compose).toContain('AI_PAGE_BUILDER_PLAYWRIGHT_MCP_URL: ${AI_PAGE_BUILDER_PLAYWRIGHT_MCP_URL:-}')
    expect(compose).toContain('AI_PAGE_BUILDER_INTERNAL_APP_ORIGIN: ${AI_PAGE_BUILDER_INTERNAL_APP_ORIGIN:-}')
  ...
  test('env example documents playwright runtime overrides', () => {
    ...
    expect(envExample).toContain('Enable the optional playwright sidecar')
  })
})
