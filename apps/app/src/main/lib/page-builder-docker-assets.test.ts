...
describe('page-builder docker assets', () => {
  test('compose defines a default internal playwright sidecar and docker runtime defaults', () => {
    ...
    expect(serverBlock).toContain('AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS: ${AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS:-}')
  ...
  test('docker assets expose page-builder public base path as runtime configuration', () => {
    ...
    expect(dockerfile).toContain('COPY --from=build /app/apps/page-builder/src/server/prod-server.mjs ./apps/page-builder/src/server/prod-server.mjs')
    expect(dockerfile).toContain('exec node apps/page-builder/src/server/prod-server.mjs')
    expect(dockerfile).not.toContain('exec bun apps/page-builder/src/server/prod-server.js')
  ...
  test('server Dockerfile uses Node 24 runtime and preserves runtime resources', () => {
    ...
    expect(releaseServerBlock).toContain('AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS: ${AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS:-}')
  ...
  test('cms verification compose keeps mock and nginx out of the default stack', () => {
    ...
    expect(verifyCompose).toContain('AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS: ${AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS:-}')
