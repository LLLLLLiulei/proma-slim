...
- [x] 2.7 在模型选项响应中增加 `selectorEnabled`，未配置模型文件或过滤后没有可用模型时返回 `false`，同时保留服务默认发送兼容。
...
- [x] 4.8 仅在 `selectorEnabled: true` 且存在可用模型时展示下拉入口；隐藏时不发送 `modelOptionId`，也不显示模型切换提示。
...
- [x] 7.5 运行未配置模型文件、无可用模型、可用多模型和隐藏选择器发送语义的聚焦回归测试。
