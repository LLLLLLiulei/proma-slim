...
系统 SHALL 在 Docker 部署资产中暴露 CMS 集成第一期需要的 PageBuilder 运行时配置，并 SHALL 保持 Docker 对外配置使用 `AI_PAGE_BUILDER_*` 命名。
...
- **THEN** 系统 SHALL 提供 `AI_PAGE_BUILDER_INTEGRATION_MODE`、`AI_PAGE_BUILDER_PUBLIC_ORIGIN`、`AI_PAGE_BUILDER_BASE_PATH`、`AI_PAGE_BUILDER_CMS_BASE_URL`、`AI_PAGE_BUILDER_INTEGRATION_SECRET`、`AI_PAGE_BUILDER_HANDOFF_TTL_MS`、`AI_PAGE_BUILDER_ACCESS_SESSION_TTL_MS` 和 `AI_PAGE_BUILDER_SYNC_EXPORT_TIMEOUT_MS` 的配置入口
...
系统 SHALL 为 `server` 提供可外部覆盖的 Docker 运行时输入，用于解析默认 Playwright sidecar MCP 地址与 page-builder 预览的内部访问 origin，而不是把这些容器网络地址写入工作区持久化配置。
...
**Reason**: 当前 Docker 资产和用户确认的部署语义是默认启动 Playwright sidecar；继续保留 profile 可选要求会与实现和后续 CMS Docker 验证拓扑冲突。

**Migration**: 使用新增的“compose 资产必须默认提供内部 Playwright MCP sidecar”要求。需要禁用或替换 sidecar 时，通过覆盖 `AI_PAGE_BUILDER_PLAYWRIGHT_MCP_URL` 和 compose 服务选择另行处理，不作为本 change 的默认路径。
