...
系统 SHALL 在 Builder 页左侧预览中，仅于页面区块选择模式启用时提供页面区块 hover 高亮与 click 选中高亮，使用户能够明确感知当前将要修改的目标区域；当当前已选区块内存在受支持的文本热点时，系统 MUST 允许后续围绕该已选区块开展内联文字编辑，而不把该区块内的热点点击误解析为新的更内层选区目标。
