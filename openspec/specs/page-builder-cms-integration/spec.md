...
系统 SHALL 支持通过 `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 启用 CMS 集成模式，并 SHALL 提供匿名只读状态接口供前端判断当前模式。系统 SHALL 支持显式配置 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS`，用于在 CMS 集成模式下开放 standalone 首页和历史入口；该配置不依赖 `NODE_ENV`，仅在显式真值时生效。
...
#### Scenario: standalone 入口配置显式生效
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 为显式真值
...
#### Scenario: 未显式开启 standalone 入口配置
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 未设置或不是显式真值
- **AND** 请求 `GET /api/integrations/cms/status`
- **THEN** 系统 SHALL 返回 `integrationMode: "cms"` 和 `enabled: true`
- **AND** 系统 SHALL NOT 返回 `devStandaloneEntryEnabled: true`
- **AND** 系统 SHALL 保持 CMS 受控入口语义
...
PageBuilder renderer 在 CMS 集成模式下 SHALL 使用 CMS integration status 判断运行模式，并 SHALL 使用 builder context 作为进入具体项目的受控前端上下文来源。仅当 status 明确返回 `devStandaloneEntryEnabled: true` 时，PageBuilder renderer MAY 复用 standalone 首页入口、历史入口和直接 builder 加载流程。
...
- **AND** 当前端同时收到 `devStandaloneEntryEnabled: true` 时，前端 SHALL 进入 standalone 入口开关生效流程
...
#### Scenario: standalone 入口开关生效时 builder 使用 standalone 上下文解析
...
#### Scenario: standalone 入口开关生效时不启用项目 API access 校验
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 为显式真值
...
#### Scenario: standalone 入口开关生效时允许未绑定 workspace 加载 CMS browser 数据
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 为显式真值
...
#### Scenario: standalone 入口开关生效时绑定 workspace 继续使用 binding 范围
- **WHEN** standalone 入口开关生效
...
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 standalone 入口开关未生效
...
#### Scenario: builder context 不使用 standalone 入口 bypass
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 为显式真值
...
- **AND** 系统 SHALL NOT 因 standalone 入口开关生效而在 builder context route 中跳过 `cmsBuilderAccess` 上下文挂载
...
系统 SHALL 提供 workspace-scoped CMS 数据读取和资产代理 API，并在 CMS 集成模式下通过统一 CMS Builder Access middleware 校验 access cookie、workspace 匹配和滑动续期。来自 `AI_PAGE_BUILDER_INTERNAL_APP_ORIGIN` 且未经过 public web/nginx 代理的 Docker Playwright 内部只读预览 GET 请求可以访问预览渲染所需的 workspace-scoped CMS 数据和资产代理；该例外不得应用到外部 public 请求、无 project binding 的 workspace、binding 内部资源缺失的 workspace 或任何状态变更请求。仅当 standalone 入口开关显式生效时，workspace-scoped CMS browser API MAY 按默认 CMS 配置或已有 project binding 范围加载数据，且仍 SHALL 只使用服务账号 token 访问 CMS `/api/*` 数据接口。

#### Scenario: CMS 模式无 access cookie 请求 workspace-scoped CMS 数据被拒绝
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 standalone 入口开关未生效
- **AND** 浏览器请求 `GET /api/workspaces/:workspaceId/page-builder/cms/sites`、`/catalogs`、`/catalogs/:catalogId`、`/contents` 或 `/assets?url=...` 时没有有效 `ai_page_builder_access` Cookie
- **THEN** 系统 SHALL 返回 `401`
...
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 standalone 入口开关未生效
- **AND** 浏览器携带的 Builder Access Session `workspaceId` 与 URL 中的 `workspaceId` 不同
- **THEN** 系统 SHALL 返回 `403`
...
系统 SHALL 在 CMS 集成模式下拒绝浏览器侧内部全量列表、本地创建和会破坏 CMS project binding 的生命周期 API；这些 API 不应因为浏览器持有 Builder Access Session 而返回全量资源或执行绑定破坏性操作。仅当 standalone 入口开关显式生效时，系统 SHALL 放行 standalone 首页和历史项目所需的全量列表、本地创建和 page-builder 历史项目删除操作。
...
#### Scenario: standalone 入口开关生效时允许读取和创建本地项目
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 为显式真值
...
#### Scenario: standalone 入口开关生效时允许删除历史 page-builder 项目
- **WHEN** `AI_PAGE_BUILDER_INTEGRATION_MODE=cms` 且 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS` 为显式真值
