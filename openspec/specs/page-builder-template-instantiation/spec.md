## Purpose
定义 PageBuilder 用户模板实例化为新项目的 API、复制范围、CMS 边界、首个 session 创建、preview state 返回和失败回滚语义。
...
- **THEN** `POST /api/page-builder/templates/:templateId/use` 可按模板使用语义创建 PageBuilder workspace、session 和 preview state

#### Scenario: CMS 集成生产模式不要求 dev standalone bypass
- **WHEN** 系统运行在 CMS 集成生产模式且未启用 `AI_PAGE_BUILDER_DEV_ALLOW_STANDALONE_ENTRY_IN_CMS`
- **THEN** 合法用户模板仍可通过 `POST /api/page-builder/templates/:templateId/use` 创建项目
