## Purpose
定义 PageBuilder 模板库一期归档前的端到端验证、自动化回归、CMS 集成边界验证、范围控制和 OpenSpec 一致性验证要求。
...
系统 SHALL 在模板库一期归档前验证 CMS 集成相关边界，明确区分 CMS 集成生产模式首页 UI 限制、CMS dev standalone bypass 和 CMS integrated Builder 当前 workspace 另存模板 API。

#### Scenario: CMS 集成生产模式开放模板库但禁用首页使用模板
- **WHEN** 系统运行在 CMS 集成生产模式
- **THEN** 首页 SHALL 挂载模板库资源区
- **AND** 首页 SHALL NOT 展示“使用模板”入口、本地 prompt 创建入口或历史记录 Tab
- **AND** 全局模板库列表、详情、预览、下载、重命名、删除和使用 API SHALL 可按模板库语义工作
