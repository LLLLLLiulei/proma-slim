...
系统 SHALL 为当前栏目展示只读的内容摘要列表，并且每一项至少包含可稳定浏览的标题、摘要和内容形状信息，而不直接暴露上游异构原始结构。

#### Scenario: 当前栏目存在内容时展示内容摘要列表
- **WHEN** 用户进入 `内容` 页签且当前栏目下存在内容
- **THEN** 系统 SHALL 展示该栏目下的内容项列表
- **AND** 每个内容项 SHALL 至少显示 `title`、`summary` 与 `shape`
- **AND** 系统 SHALL 展示与该内容项相关的素材计数摘要
...
系统 SHALL 通过宿主管理的 CMS 读取链路向 Builder 浏览弹框提供栏目与内容数据，并在缺少配置或上游失败时返回可操作的浏览错误，而不是要求用户在 UI 中提供原始 CMS 凭据。

#### Scenario: 宿主配置有效时可以读取栏目和内容
- **WHEN** 宿主 CMS 配置有效且上游请求成功
- **THEN** 系统 SHALL 在弹框中加载栏目树与内容摘要数据
- **AND** 系统 SHALL 不要求用户在浏览弹框中输入 `ZUSID`、`CurrentSite` 或其他原始鉴权信息

#### Scenario: CMS 数据读取失败时在弹框内展示错误
- **WHEN** 宿主 CMS 配置缺失、登录态失效或上游请求失败
- **THEN** 系统 SHALL 在弹框内展示明确的错误提示和重试入口
- **AND** 系统 SHALL 不在错误内容中泄露 Cookie 值或完整请求头
