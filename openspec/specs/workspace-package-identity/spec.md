## Purpose
Defines the repository root package identity, active workspace package scope, and package-rename compatibility boundary for AI Page Builder.
...
All active workspace package names SHALL use the `@ai-page-builder/*` scope instead of the historical Proma package scope.
...
- **AND** no active workspace package name SHALL use the historical Proma package scope
...
- **AND** it SHALL NOT depend on the historical Proma package scope
...
- **AND** active source code SHALL NOT import those packages through the historical Proma package scope
...
- **WHEN** archive changes, historical analysis documents or runtime namespace documentation mention Proma package names, `PROMA_*`, `~/.proma`, `data-proma-*`, `__PROMA_*` or `proma:*`
