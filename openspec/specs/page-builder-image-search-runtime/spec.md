## Purpose
定义 page-builder 会话中的宿主侧图片搜索运行时能力，包括 query 级注入的 `image_search` SDK MCP server、多 provider 图片搜索结果归一化、provider 诊断、详细 MCP 执行日志，以及导入当前工作区 `assets/` 的受控资产导入与图片优化语义。
