## Purpose
定义 PageBuilder 首页模板库 Tab、模板列表状态、预览、使用模板跳转、删除用户模板、CMS 集成模式下模板库可见但禁用首页使用模板，以及历史记录不回退的用户可见行为。
