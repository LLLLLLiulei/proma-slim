...
export function setupConsoleRedirection() {
    ...
    // - If env PAGEBUILDER_MCP_LOG_PATH is set, use it
    // - Otherwise use user home directory: ~/.pagebuilder-mcp-server/pagebuilder-mcp-server-YYYY-MM-DD.log
    const resolveLogFilePath = () => {
        const envPath = process.env.PAGEBUILDER_MCP_LOG_PATH;
        ...
        return path.join(homeDir, '.pagebuilder-mcp-server', `pagebuilder-mcp-server-${dateStr}.log`);
    };
