...
export class BaseImageAnalysisService {
    ...
    async processImageSource(imageSource) {
        const isAliyun = configurationService.getPlatformMode() === 'ALIYUN';
        // Validate image source and size
        await this.fileService.validateImageSource(imageSource, isAliyun ? Infinity : this.MAX_IMAGE_SIZE_MB);
        ...
        if (this.fileService.isUrl(imageSource)) {
            ...
            const imageData = await this.fileService.encodeImageToBase64(imageSource, isAliyun
                ? {
                    compressIfOverBytes: this.fileService.ALIYUN_DIRECT_BASE64_IMAGE_LIMIT_BYTES,
                    maxBase64Bytes: this.fileService.ALIYUN_MAX_BASE64_IMAGE_BYTES
                }
                : {});
    ...
    async executeVisionAnalysis(systemPrompt, userPrompt, imageContents, toolName) {
        try {
            const prompt = configurationService.getPlatformMode() === 'ALIYUN'
                ? `${systemPrompt}\n\nUser request:\n${userPrompt}`
                : userPrompt;
            const messages = configurationService.getPlatformMode() === 'ALIYUN'
                ? createMultiModalMessage(imageContents, prompt)
                : [
                    {
                        role: 'system',
